...
export const Dropdown: React.FC<DropdownProps> = ({
  ...
  return (
    <DropdownMenu.Root open={open} onOpenChange={onOpenChange}>
      ...
      <StyledContent {...rest}>
        <DropdownMenu.Group className="flex flex-col gap-y-1.5">
...
const StyledContent = styled(DropdownMenu.Content).attrs({
  className: 'bg-neutral-0 rounded-xl p-2 shadow-xl' as string,
})``;

const StyledItem = styled(DropdownMenu.Item).attrs({
  className:
    'rounded-xl outline-none focus:outline-none focus:ring focus:ring-primary-200',
})`
  &:hover:focus {
    outline: none;
    box-shadow: none;
