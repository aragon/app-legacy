...
> [!WARNING]
> This repository is no longer maintained. The new application can be found at [https://github.com/aragon/app](https://github.com/aragon/app).
...
| **Preview**    | [Generated by the pipelines]      | Manual by adding the `preview` label to PRs | `.env.staging`     |
| **Develop**    | https://dev.app-legacy.aragon.org | Automatic on `develop` branch push          | `.env.staging`     |
| **Staging**    | https://stg.app-legacy.aragon.org | Automatic on `main` branch push             | `.env.production`  |
| **Production** | https://app-legacy.aragon.org     | Manual in coordination with @devops team    | `.env.production`  |
